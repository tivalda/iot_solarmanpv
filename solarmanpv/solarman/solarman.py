...
def run(config):
    ...
    mongo_db.mongo_insert(topic, "station", json.dumps(station_data))
    mongo_db.mongo_insert(topic, "inverter", json.dumps(inverter_data))
    mongo_db.mongo_insert(topic, "logger", json.dumps(logger_data))
    mongo_db.mongo_insert(topic, "inverter_list", json.dumps(inverter_data_list))
    mongo_db.mongo_insert(topic, "logger_list", json.dumps(logger_data_list))
    ...
    client = mqtt.connect_mqtt(config["mqtt"]["broker"], config["mqtt"]["port"], config["mqtt"]["username"], config["mqtt"]["password"])
    ...
    if inverter_device_state == 1:
        ...
        for i in station_data:
            if station_data[i]:
                if i not in discard:
                    mqtt.message(config["mqtt"], topic + "/station/" + i, station_data[i], config["debug"], client)
        print(f"{time_stamp()}: ⚡ Sending inverter data to mqtt")
        for i in inverter_data:
            if inverter_data[i]:
                if i not in discard:
                    mqtt.message(config["mqtt"], topic + "/inverter/" + i, inverter_data[i], config["debug"], client)
        print(f"{time_stamp()}: ⚡ Sending inverter data list to mqtt")
        if inverter_data_list:
            mqtt.message(config["mqtt"], topic + "/inverter/attributes", json.dumps(inverter_data_list), config["debug"], client)
        print(f"{time_stamp()}: ⚡ Sending logger data to mqtt")
        for i in logger_data:
            if logger_data[i]:
                if i not in discard:
                    mqtt.message(config["mqtt"], topic + "/logger/" + i, logger_data[i], config["debug"], client)
        print(f"{time_stamp()}: ⚡ Sending logger data list to mqtt")
        if logger_data_list:
            mqtt.message(config["mqtt"], topic + "/logger/attributes", json.dumps(logger_data_list), config["debug"], client)
        ...
        mqtt.message(config["mqtt"], topic + "/inverter/deviceState", inverter_data["deviceState"], config["debug"], client)
        mqtt.message(config["mqtt"], topic + "/logger/deviceState", logger_data["deviceState"], config["debug"], client)
